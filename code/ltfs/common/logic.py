...
def if_else(d: int, 
    ...
    for k, v in S_index_A.items():
        try:
            ...
            layer.W3[index[C], S_index_A[k]] += ifINF
            layer.W3[index["B_global"], S_index_A[k]] += -ifINF
            layer.W3[index["B_local"], S_index_A[k]] += -ifINF
            s_idx += v
    ...
    for k, v in {k: len_slice(index[k]) for k in A_list}.items():
        ...
        layer.W3[index[C], S_index_A[k_S]] += ifINF
        layer.W3[index["B_global"], S_index_A[k_S]] += -ifINF
        layer.W3[index["B_local"], S_index_A[k_S]] += -ifINF
        s_idx += v
    ...
    for k, v in S_index_B.items():
        ...
        layer.W3[index[C], S_index_B[k]] += -ifINF
        s_idx += v
    ...
    for k, v in {k: len_slice(index[k]) for k in B_list}.items():
        ...
        layer.W3[index[C], S_index_B[k_S]] += -ifINF
        s_idx += v
...
def less_than(d: int, index: dict, a: str, b: str, target: str, grad: bool = False, s_idx: int = None, tol=1e-7):
