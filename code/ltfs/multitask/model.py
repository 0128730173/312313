...
class Model(BaseLayer):
    ...
    def _build_model(self, d: int, m: int, index: dict, grad: bool):
        ...
        self._less_than = BaseTransformer.from_pretrained(
            empty_attention(),
            common.logic.less_than(d, index, "A_dist", "D", "S_is_less", grad=grad)[0],
        )
        ...
        self._select_flag = BaseTransformer.from_pretrained(
            empty_attention(),
            common.logic.if_else(d, index, ["order", "S_change"],
                ["A_dist", "S_is_less"], "bin_dfs", ["S_D", "S_update"], grad=grad)[0]
        )

        # step 22
        self._if_else = BaseTransformer.from_pretrained(
            empty_attention(),
            common.logic.if_else(d, index, ["S_D", "P_n"],
                ["D", "OUT"], "S_update", ["D", "OUT"], grad=grad)[0]
        )
