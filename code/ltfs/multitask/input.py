...
def build_input(n: int, start: int, bfs: bool = False):
    hstack = {}
    rows = n + 1
    ...
    hstack["bin_dfs"] = torch.zeros(rows, 1)
    hstack["bin_dfs"][0] = int(bfs)

    # adjacency matrix
    hstack["A_row"] = torch.zeros(rows, 1)
    hstack["A_is_zero"] = torch.zeros(rows, 1)
    hstack["A_dist"] = torch.zeros(rows, 1)
    ...
    hstack["S_update"] = torch.zeros(rows, 1)
    hstack["S_D"] = torch.zeros(rows, 1)
