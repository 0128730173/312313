...
def mask_write(d: int, index: dict, grad: bool = False, s_idx: int = 0):
    with torch.no_grad():
        ...
        layer.W1[index["bin_disc"], index["S_change"]] = -1
...
def interrupt_1(d: int, index: dict, grad: bool = False, s_idx: int = 0):
    with torch.no_grad():
        ...
        layer.W1[index["bin_disc"], s1] = -1
        layer.W1[index["bin_visit"], s2] = -1
        layer.W1[index["bin_disc"], s2] = 1
        layer.W1[index["B_local"], s3] = 1
        layer.W1[index["bin_disc_all"], index["bin_disc_all"]] = 1

        layer.W2[s1, s1] = -1
        layer.W2[s2, s1] = -1
        layer.W2[s3, s1] = 1
        layer.W2[index["bin_disc_all"], index["bin_disc_all"]] = 1

        layer.W3[s1, s1] = 1
        layer.W3[index["bin_disc_all"], index["bin_disc_all"]] = 1

        layer.W4[s1, index["bin_disc_all"]] = 1
        layer.W4[index["bin_disc_all"], index["bin_disc_all"]] = -1
...
def _termination_attn(d: int, m: int, index: dict, grad: bool = False):
    with torch.no_grad():
        ...
        layer.W2[index["bin_disc"], index["S_bin_term_1"]] = config.INF
        layer.W2[index["bin_disc_all"], index["S_bin_term_2"]] = config.INF
...
def _update_attn(d: int, m: int, index: dict, grad: bool = False):
    with torch.no_grad():
        ...
        layer.W1[index["bin_write"], index["bin_disc"]] = 2
        ...
        layer.W2[index["B_global"], index["bin_disc"]] = -config.INF
...
def _update_mlp(d: int, index: dict, grad: bool = False, s_idx: int = 0):
    with torch.no_grad():
        ...
        for field in ["bin_visit", "bin_disc"]:
            field_size = len_slice(index[field])
        ...
        layer.W2[index["S_change"], index["S_change"]] = 1
        layer.W3[index["S_change"], index["S_change"]] = 1
        layer.W4[index["S_change"], index["bin_disc"]] = 1
